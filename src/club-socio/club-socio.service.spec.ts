...
describe('ClubSocioService', () => {
  ...
  it('addSocio debería asociar un socio a un club', async () => {
    ...
    const result: ClubEntity = await service.addMemberToClub(club.id, socio.id);
  ...
  it('deleteMemberFromClub debería eliminar un socio de un club', async () => {
    ...
    const clubSocios: SocioEntity[] = await service.findMembersFromClub(
      club.id,
    );
  ...
  it('updateMembersFromClub debería actualizar los miembros de un club', async () => {
    ...
    for (let i: number = 0; i < 5; i++) {
      ...
      expect(updatedClub.socios[i].fechaNacimiento).toStrictEqual(
        newSocios[i].fechaNacimiento,
      );
    }
  });
});
